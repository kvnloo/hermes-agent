...
def test_large_retained_listing_does_not_full_read_receipt(tmp_path, monkeypatch):
    """The listing path must not hydrate a large writer-shaped receipt in full."""
    ...
    monkeypatch.setenv("HERMES_SESSION_ID", owner)
    home = get_hermes_home()
    path = _write_receipt(home, "proc_tailbounded", "x" * 200_000, owner)

    original_read_text = Path.read_text

    def forbid_full_receipt_read(self, *args, **kwargs):
        if self == path:
            raise AssertionError("retained listing attempted a full receipt read")
        return original_read_text(self, *args, **kwargs)

    monkeypatch.setattr(Path, "read_text", forbid_full_receipt_read)

    rows = ProcessRegistry().list_sessions(include_retained=True)

    row = next(item for item in rows if item["session_id"] == "proc_tailbounded")
    assert row["output_preview"] == "x" * 200
