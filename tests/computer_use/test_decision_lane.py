"""Tests for the System-One decision lane substrate (#113850)."""
...
def _cand(ref, label="", enabled=True, role="button"):
    return ElementCandidate(ref=ref, label=label, enabled=enabled, role=role)
...
def test_rules_abstain_falls_through_to_reranker():
    ...
    decision, packet = run_decision_lane(
        SemanticState(goal_hint="find settings"), (_cand("b1", "x"), _cand("b2", "y")),
        reranker=reranker)
...
def test_default_verifier_rejects_missing_target_for_click():
    decider = lambda state, cands: Decision(action="click", confidence=0.95, backend="aux")
    decision, packet = run_decision_lane(
        SemanticState(), (_cand("b1", "x"),), aux=decider)
    assert decision is None
    assert packet.verifier_outcome == "veto"
...
def test_packet_round_trip_and_no_pixels_or_secrets():
    ...
    _, packet = run_decision_lane(
        SemanticState(goal_hint="name"), (_cand("f1", "Name", role="textfield"),), aux=decider)
