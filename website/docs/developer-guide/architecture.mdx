...
The explorer follows one request from a user-facing surface through routing,
prompt assembly, the agent/provider/tool loop, delivery, and durable state.
Select a stage for canonical implementation and test links. A complete text
representation is included below the interactive canvas and in `llms-full.txt`.

<ArchitectureExplorer />

<details>
<summary>Complete request lifecycle (text)</summary>

1. **Surfaces** — CLI, TUI, Desktop, ACP, batch, API, cron, and messaging
   accept work. Each surface normalizes user intent before entering the shared
   agent runtime; surface-specific presentation stays outside the core loop.
2. **Session / Gateway** — routing, authorization, history, interrupts, and
   session identity converge. Gateway adapters normalize platform events and
   resolve a durable session key; direct surfaces restore the same conversation
   model without gateway routing.
3. **Prompt Assembly** — stable instructions, project context, skills, memory,
   and history form the request. Ordered prompt tiers preserve a byte-stable
   cached prefix, with compression as the controlled context-budget exception.
4. **Agent Loop** — `AIAgent` coordinates model calls, tool turns, iteration
   budgets, retries, fallback, interrupts, and completion while preserving
   strict message-role alternation.
5. **Provider Runtime** — provider, model, credentials, API mode, and fallback
   route resolve for the call. Chat Completions, Codex Responses, and Anthropic
   Messages retain their provider-specific wire behavior.
6. **Tools / Approvals** — toolsets and availability checks determine the
   model-visible schema. The registry dispatches handlers while approval gates
   hold risky terminal actions for an explicit decision.
7. **Events / Delivery** — streaming deltas, tool progress, final output, and
   hooks return through surface callbacks. Gateway delivery handles platform
   limits, edits, media, streaming, and final-message reconciliation.
8. **Sessions / Memory / Usage / State** — SQLite persists messages, lineage,
   per-model usage, routing, and searchable state. Memory providers and session
   search add recall without mutating the stable prompt prefix mid-session.
9. **Next Turn** — completed output becomes history and queued input re-enters
   the same lifecycle through session routing and prompt assembly.
