...
- **Credentials inside mined commands are masked** (`ghp_…`, `bot<id>:<token>`
  ...
  bearer tokens) in both the printed `e.g.` examples and the `--json` payload,
  using the same redactor as terminal output. Masked text is never used as an
  allowlist pattern: a command whose glob would embed a credential
  (`TOKEN=… git …`) is proposed under its dangerous-class key instead. The
  session database itself still holds the command as it was executed.
