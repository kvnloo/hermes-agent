...
| `post_api_request` | Observer | After normalized provider success; return ignored. | `task_id`, `turn_id`, `api_request_id`, `session_id`, `platform`, `model`, `provider`, `base_url`, `api_mode`, `api_call_count`, `api_duration`, `started_at`, `ended_at`, `first_chunk_at`, `first_reasoning_at`, `first_text_at`, `finish_reason`, `message_count`, `response_model`, `response`, `usage`, `assistant_message`, `assistant_content_chars`, `assistant_tool_call_count` | The three `first_*` fields separate first stream activity, first reasoning, and first visible text; each is `None` when that streamed event never occurred. Sanitized `response` is available, but raw normalized `assistant_message` may contain model/user content; `usage` is accounting data. |
